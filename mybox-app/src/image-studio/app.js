...
export function createImageStudioApp({ generator = null } = {}) {
  const performGeneration = async (input, context) => {
    const state = await load(context.storage);
    const locals = state.templates.filter((item) => item.state === "active");
    const connected = await context.connections.pull("image-studio.prompt-library").catch(() => ({ items: [] }));
    ...
    try {
      ...
      await context.emit("image-studio.generation.completed", {
      ...
      return { generation: completed.generation };
    } catch (error) {
  ...
  return defineApp({
    ...
    handlers: {
      async "image-studio.template.list"(_, { storage, connections }) { const state = await load(storage); const connected = await connections.pull("image-studio.prompt-library").catch(() => ({ items: [], failures: [] })); return { templates: [...BUILT_IN_TEMPLATES, ...state.templates, ...connected.items], failures: connected.failures }; },
