...
function PromptPanel({ prompt, selections, ratio, customized, noteAvailable, onChange, onReset, onImportFile, onOpenPage }) {
  const [importOpen, setImportOpen] = useState(false);
  const importRootRef = useRef(null);
  const fileInputRef = useRef(null);
  useEffect(() => {
    if (!importOpen) return undefined;
    const close = (event) => { if (!importRootRef.current?.contains(event.target)) setImportOpen(false); };
    const escape = (event) => { if (event.key === "Escape") setImportOpen(false); };
  ...
  return <aside className="image-prompt-panel" id="image-prompt-panel" aria-labelledby="image-prompt-panel-title">
    <header>
      ...
      <div className="image-prompt-actions">
        ...
        {customized && <button type="button" aria-label="選択内容から全体Promptを再構築" data-tooltip="再構築" onClick={onReset}><ArrowsClockwise size={17} /></button>}
        <div className="image-prompt-import-root" ref={importRootRef}>
          <button type="button" aria-label="全体Promptをインポート" data-tooltip="インポート" aria-haspopup="menu" aria-expanded={importOpen} aria-controls="image-prompt-import-menu" onClick={() => setImportOpen((open) => !open)}><UploadSimple size={17} /></button>
          {importOpen && <div className="image-prompt-import-menu" id="image-prompt-import-menu" role="menu" aria-label="全体Promptのインポート元">
            <button type="button" role="menuitem" onClick={() => { setImportOpen(false); fileInputRef.current?.click(); }}><FileText size={18} /><span>ファイル</span></button>
            {noteAvailable && <button type="button" role="menuitem" onClick={() => { setImportOpen(false); onOpenPage(); }}><Notebook size={18} /><span>Note Page</span></button>}
          </div>}
          <input ref={fileInputRef} className="image-prompt-file-input" type="file" tabIndex="-1" accept=".md,.markdown,.txt,text/markdown,text/plain" onChange={async (event) => { const file = event.target.files?.[0]; if (file) await onImportFile(file); event.target.value = ""; }} />
        </div>
      </div>
    </header>
    ...
    <textarea aria-label="全体Prompt" value={prompt} maxLength={MAX_PROMPT_LENGTH} spellCheck="false" onChange={(event) => onChange(event.target.value)} />
  </aside>;
}
...
function NotePageDialog({ client, onImport, onClose }) {
  ...
  useEffect(() => {
    if (!projectId) { setPages([]); setTags([]); return undefined; }
  ...
  return <div className="image-dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="image-template-dialog image-page-dialog" role="dialog" aria-modal="true" aria-labelledby="image-page-import-title">
      <header><h2 id="image-page-import-title">Note Page</h2><button type="button" aria-label="閉じる" data-tooltip="閉じる" onClick={onClose}><X size={20} /></button></header>
      {error && <p className="image-error" role="alert">{error}</p>}
      {projects.length > 0 && <ThemedSelect id="image-note-project" label="Project" options={projects.map((project) => ({ id: project.id, label: project.name }))} value={projectId} onChange={setProjectId} placement="bottom" />}
      <label className="image-note-page-search" htmlFor="image-note-page-query">
        <span>検索</span>
        <span><MagnifyingGlass size={17} aria-hidden="true" /><input id="image-note-page-query" type="search" value={query} placeholder="タイトル・Tag" autoComplete="off" autoFocus onChange={(event) => setQuery(event.target.value)} /></span>
      </label>
      <span className="sr-only" role="status">{busy ? "Pageを読み込み中" : `${filteredPages.length}件のPage`}</span>
      <div className="image-note-page-list" aria-busy={busy}>{busy ? <span className="image-spinner" role="status" aria-label="Pageを読み込み中" /> : filteredPages.length ? filteredPages.map((page) => <button type="button" key={page.id} onClick={() => importPage(page)}><Notebook size={18} aria-hidden="true" /><span><strong>{page.title}</strong><small>{page.tags.length ? page.tags.map((tag) => `#${tag}`).join("  ") : page.updatedAt ? new Date(page.updatedAt).toLocaleDateString("ja-JP") : ""}</small></span></button>) : <p>{query.trim() ? "一致するPageがありません" : "Pageがありません"}</p>}</div>
    </section>
  </div>;
}
...
export function ImageStudioView({ onOpenPage, desktop = false, profileId = "local-user", persistenceReady = true, onClose, onOpenSettings, onToggleAssistant, assistantOpen = false, onContextChange, onToast = () => {}, appRuntime = null }) {
  ...
  const selectGeneration = (generation) => { setPromptSource(generation.input?.promptSource ?? null); setReferences(generation.input?.references ?? []); setReferenceInstruction(generation.input?.referenceInstruction ?? "");
    ...
    setSelectedId(generation.id); setHistoryOpen(false); if (generation.input) { setSubject(generation.input.subject ?? ""); setSelections({ ...emptySelections, ...(generation.input.selections ?? {}) }); setRatio(generation.input.ratio ?? "auto"); setExtra(generation.input.extra ?? ""); setPromptOverride(generation.finalPrompt ?? generation.input.promptOverride ?? null); } };
