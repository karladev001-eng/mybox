...
function NotePageDialog({ client, onImport, onClose }) {
  ...
  const importPage = async (page) => {
    ...
    try { const result = await client.readNotePageMarkdown(projectId, page.id); onImport(result.markdown); onClose(); }
...
export function ImageStudioView({ desktop = false, profileId = "local-user", persistenceReady = true, onClose, onOpenSettings, onToggleAssistant, assistantOpen = false, onContextChange, onToast = () => {}, appRuntime = null }) {
  ...
  const generate = async () => {
    ...
    setBusy(true); setError("");
    try {
      const result = await client.generate({ subject: subject.trim(), selections, ratio, references, referenceInstruction, extra, ...(promptOverride !== null ? { promptOverride: customPrompt } : {}) });
  ...
  const selectGeneration = (generation) => { setSelectedId(generation.id); setHistoryOpen(false); if (generation.input) { setSubject(generation.input.subject ?? ""); setSelections({ ...emptySelections, ...(generation.input.selections ?? {}) }); setRatio(generation.input.ratio ?? "auto"); setExtra(generation.input.extra ?? ""); setPromptOverride(generation.finalPrompt ?? generation.input.promptOverride ?? null); } };
