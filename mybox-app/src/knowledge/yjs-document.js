...
export function seedPage(doc, page) {
  doc.transact(() => {
    const map = new Y.Map();
    for (const key of ["imageRecord", "folderId", "file", "kind", "recordVersion", "context", "provenance", "legacyContextUnavailable", "sessionMetadata", "createdAt", "updatedAt"]) if (page[key] !== undefined) map.set(key, structuredClone(page[key]));
...
export function readPage(doc, pageId) {
  ...
  return {
    id: pageId,
    ...Object.fromEntries(["imageRecord", "folderId", "file", "kind", "recordVersion", "context", "provenance", "legacyContextUnavailable", "sessionMetadata", "createdAt", "updatedAt"].filter((key) => page.has(key)).map((key) => [key, structuredClone(page.get(key))])),
...
export function commitRecordPage(doc, value) {
  ...
  doc.transact(() => {
    for (const key of ["imageRecord", "folderId", "file", "title", "kind", "recordVersion", "context", "provenance", "legacyContextUnavailable", "sessionMetadata", "createdAt", "updatedAt"]) if (value[key] !== undefined) existing.set(key, structuredClone(value[key]));
